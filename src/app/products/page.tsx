...
// Tambahkan Caching (Biar kalau balik ke sini halamannya langsung muncul 0 detik)
...
export default async function ProductsPage({
  ...
  // Ambil data secara PARALLEL agar lebih cepat
  ...
  // Ambil profil dan produk secara parallel juga
  const [user, products] = await Promise.all([
    authUser ? prisma.profile.findUnique({ where: { userId: authUser.id } }) : null,
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/5 bg-background/50 backdrop-blur-xl">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-20 items-center justify-between">
            <Link href="/" className="group flex items-center justify-center transition-transform group-hover:scale-110">
              <Image
                src="/icons/icons-120x40.jpg"
                alt="SummitXGear Logo"
                width={120}
                height={40}
            ...
            <div className="flex items-center gap-4">
              <div className="relative hidden md:block">
                <input
                  ...
                  className="w-64 rounded-full border border-white/10 bg-primary/20 py-2 pl-4 pr-10 text-sm transition-all focus:border-secondary/30 focus:outline-none focus:ring-1 focus:ring-secondary/30"
                />
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <Link href="/profile" className="h-8 w-8 rounded-full bg-gradient-to-tr from-secondary to-accent border border-white/10 transition-transform hover:scale-110 active:scale-95" />
                </div>
              ) : (
      ...
      <main className="mx-auto max-w-7xl px-6 py-8 md:py-12 lg:px-8">
        {/* Page Hero/Header */}
        <div className="mb-8 md:mb-12 space-y-3 md:space-y-4">
          ...
          <p className="max-w-2xl text-base md:text-lg text-zinc-400 font-light leading-relaxed">
          ...
          <Link
            ...
            className={`whitespace-nowrap rounded-full px-6 py-2.5 text-sm font-medium transition-all ${
              !selectedCategory 
                ...
                : 'border border-white/10 bg-primary/30 hover:bg-primary/50'
            }`}
          >
            All Gear
          </Link>
        ...
        {products.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-[3rem] border border-white/5 bg-primary/10 py-32 text-center backdrop-blur-sm">
            ...
            {user?.role === 'ADMIN' && (
              <Link href="/admin/products/new" className="mt-8 rounded-full bg-secondary px-8 py-3 text-sm font-bold text-white hover:bg-secondary/80 shadow-lg shadow-secondary/20">
                Add First Product
              </Link>
            )}
          </div>
