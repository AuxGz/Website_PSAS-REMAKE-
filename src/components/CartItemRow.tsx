...
export default function CartItemRow({ item }: CartItemRowProps) {
  ...
  return (
    <div className={`flex items-center gap-6 py-6 border-b border-white/5 transition-opacity ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
      {/* Product Image */}
      ...
      <div className="flex-1 space-y-1">
        ...
        <p className="text-sm text-zinc-500">Unit Price: Rp {Number(item.product.price).toLocaleString('id-ID')}</p>
        <button onClick={handleRemove} className="text-[10px] tracking-widest uppercase text-red-500/60 hover:text-red-500 transition-colors mt-2">
          Remove
        </button>
      </div>
